...
export default function CvPreviewModal({
  ...
  const fullName = cvData.personalInfo.fullName || personName;
  ...
  return (
    ...
    >
      <div className="relative flex flex-col w-full max-w-4xl h-[94vh] sm:h-[90vh] bg-card border border-main rounded-2xl shadow-2xl overflow-hidden animate-scale-up">
        ...
        <div className="no-print shrink-0 flex items-center justify-between px-3 sm:px-6 py-3 border-b border-main bg-card/95 backdrop-blur-md z-10 gap-2">
          {/* Left: Document Info */}
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            ...
            <Link
              ...
              className="hidden sm:inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border border-main text-sub hover:text-main hover:bg-card-hover text-xs font-medium transition-colors"
            >
              <ExternalLink className="w-3.5 h-3.5" />
              <span>{t('Halaman Penuh', 'Full Page')}</span>
            </Link>
            ...
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3.5 sm:px-4 py-1.5 sm:py-2 rounded-lg font-bold text-xs sm:text-sm text-white bg-blue-600 hover:bg-blue-700 active:scale-98 disabled:opacity-75 transition-all shadow-md shadow-blue-500/20 cursor-pointer"
            >
              {isExporting ? (
                <>
            ...
            <button
              onClick={closeCvModal}
              className="p-1.5 sm:p-2 rounded-lg text-sub hover:text-main hover:bg-card-hover border border-transparent hover:border-main transition-colors ml-0.5"
        ...
        <div className="flex-1 overflow-y-auto overflow-x-auto p-4 sm:p-6 md:p-8 bg-slate-100/70 dark:bg-slate-950/80 flex flex-col items-center select-text">
          {isLoading ? (
        ...
        <div className="no-print sm:hidden shrink-0 flex items-center justify-between px-4 py-2.5 bg-card border-t border-main">
          {/* Mobile Zoom Controls */}
          <div className="flex items-center rounded-lg border border-main bg-card p-0.5 text-xs font-medium text-main">
            <button
              type="button"
              onClick={handleZoomOut}
              disabled={zoomLevel <= 0.65}
